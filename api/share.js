...
export default async function handler(req,res){
  try{
    ...
    const question=d?.question||'Which should I choose?',proto=req.headers['x-forwarded-proto']||'https',host=req.headers.host,origin=`${proto}://${host}`,canonical=`${origin}/d/${token}`,image=`${origin}/api/og?token=${encodeURIComponent(token)}&v=4`,title=`Help me decide: ${question}`,desc=`${a} vs ${b} — vote on Tiebreak.`;
