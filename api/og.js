...
function font(){
  ...
  const fontPath=require.resolve('@fontsource/inter/files/inter-latin-700-normal.woff2');
  interBold=fontkit.create(fs.readFileSync(fontPath));
  return interBold;
}
...
function textPath(text,x,y,size,fill='#fff',center=false){
  const f=font(),run=f.layout(clean(text)),scale=size/f.unitsPerEm;
  const width=run.positions.reduce((n,p)=>n+p.xAdvance,0)*scale;
  let pen=center?x-width/2:x,svg='';
  for(let i=0;i<run.glyphs.length;i++){
    const g=run.glyphs[i],p=run.positions[i];
    const gx=pen+(p.xOffset||0)*scale,gy=y-(p.yOffset||0)*scale;
    svg+=`<g transform="translate(${gx.toFixed(2)} ${gy.toFixed(2)}) scale(${scale.toFixed(6)} ${(-scale).toFixed(6)})">${g.path.toSVG()}</g>`;
    pen+=p.xAdvance*scale;
  }
  return `<g fill="${fill}">${svg}</g>`;
}
export default async function handler(req,res){
 try{
  ...
  const base=Buffer.from(`<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg"><rect width="1200" height="630" fill="#0b0d17"/>${textPath('TIEBREAK',60,62,26,'#c4b5fd')}${textPath(q1,60,120,42)}${q2?textPath(q2,60,166,42):''}<rect x="70" y="220" width="500" height="315" rx="22" fill="#fff"/><rect x="630" y="220" width="500" height="315" rx="22" fill="#fff"/></svg>`);
  const photos=await sharp(base).composite(composites).png().toBuffer();
  const overlay=Buffer.from(`<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg"><circle cx="110" cy="260" r="30" fill="#8b5cf6"/>${textPath('A',110,270,28,'#fff',true)}<circle cx="670" cy="260" r="30" fill="#8b5cf6"/>${textPath('B',670,270,28,'#fff',true)}<rect x="390" y="555" width="420" height="56" rx="28" fill="#8b5cf6"/>${textPath('VOTE ON TIEBREAK',600,591,22,'#fff',true)}</svg>`);
 ...
 }catch(e){console.error('OG image error',e);res.status(404).end()}
}
