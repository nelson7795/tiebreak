...
// Tiny built-in 5x7 vector alphabet. Text is drawn as SVG rectangles instead of
// relying on fonts installed in the Vercel runtime, so social-card text cannot disappear.
const G={
...
function vtext(text,x,y,size,color='#fff',center=false){
 const s=Math.max(2,size/7), gap=s, advance=6*s, str=clean(text).toUpperCase();
 let start=x;if(center)start=x-(str.length*advance-gap)/2;
 let out='';
 [...str].forEach((ch,i)=>{const rows=G[ch]||G['?'];rows.forEach((row,ry)=>[...row].forEach((on,rx)=>{if(on==='1')out+=`<rect x="${start+i*advance+rx*s}" y="${y+ry*s}" width="${s+.15}" height="${s+.15}" rx="${Math.min(1,s/4)}" fill="${color}"/>`}))});
 return out;
}
